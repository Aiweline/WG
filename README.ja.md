...
| <code>cmd</code> / <code>ui</code> / <code>scripts</code> | 安全な control core、5 画面のクライアント UI、実 TCP/UDP プロキシスクリプト |

安全な control core はアーキテクチャと管理フローを検証します。実際のプロキシ通信は <code>wg-proxy</code> が処理します。
...
「接続」画面はモックではなく実際に動作します。`server-cert.pem` と `token`（権限 `0600`）を `~/.wg-client` に配置し、`./bin/wg-client-ui` を起動して、サーバー IP、ポート（既定値 `9518`）、TCP/UDP モードを追加または選択します。UI はサーバープロファイルをローカルに保存し、同じディレクトリの `wg-proxy` を `127.0.0.1:47101`（TCP）および／または `127.0.0.1:47102`（UDP）で起動します。システム DNS は変更しません。
...
実プロキシのデータプレーンでは、バイナリをビルドしてスクリプト切替を使用します。
...
> [!IMPORTANT]
プロキシインストーラーは <code>wg-proxy-tcp.service</code> と <code>wg-proxy-udp.service</code> を有効化します。DNS、ルート、NAT、ファイアウォールは変更しません。
